...
namespace slu { struct StackItemRef; }//Forward decl

//Turns the item on the top of the stack into a StackItemRef
slu::StackItemRef Slu_makeStackItemRef(lua_State* _L, const lua_Integer key, const int parentIdx);
//Turns the item on the top of the stack into a StackItemRef
slu::StackItemRef Slu_makeStackItemRef(lua_State* _L, const std::string& key, const int parentIdx);
...
namespace slu
{
	...
	struct StackItem
	{
		...
		//normal
		const StackItem& set(const slu::Pushable auto& newVal, const lua_Integer key)const
		{
			slu::push(newVal);
			lua_seti(L, idx, key);
			return *this;
		...
		//normal
		const StackItem& set(const slu::Pushable auto& newVal, const std::string& key)const
		{
			slu::push(key);
			slu::push(newVal);
			lua_settable(L, idx);
		...
		slu::StackItemRef get(const lua_Integer key)const;
		//normal, but will raw-set by default
		slu::StackItemRef get(const std::string& key)const;

		//raw
		slu::StackItemRef operator[](const lua_Integer key)const;
		//raw
		slu::StackItemRef operator[](const std::string& key)const;
		...
		const StackItem& setStack(const slu::Pushable auto& newVal)const
		{
			slu::push(L, newVal);
		...
		//raw
		const StackItem& operator=(const slu::Pushable auto& newVal)const {
			setStack(newVal);
			return *this;
		}
	...
	struct StackItemRef : StackItem
	{
		...
		//normal
		StackItem& set(const slu::Pushable auto& newVal)
		{
			setStack(newVal);
		...
		//raw
		StackItem& operator=(const slu::Pushable auto& newVal)
		{
	...
	//normal, but will raw-set by default
	inline slu::StackItemRef slu::StackItem::get(const lua_Integer key) const
	{
		lua_geti(L, idx, key);
		return Slu_makeStackItemRef(L, key, idx);
	}
	//normal, but will raw-set by default
	inline slu::StackItemRef slu::StackItem::get(const std::string& key)const
	{
	...
	//raw
	slu::StackItemRef slu::StackItem::operator[](const lua_Integer key)const
	{
		lua_rawgeti(L, idx, key);
		return Slu_makeStackItemRef(L, key, idx);
	}
	//raw
	slu::StackItemRef slu::StackItem::operator[](const std::string& key)const
	{
		lua_pushlstring(L, key.data(), key.size());
		lua_rawget(L, idx);
		return Slu_makeStackItemRef(L, key, idx);
...
slu::StackItemRef Slu_makeStackItemRef(lua_State* L, const lua_Integer key, const int parentIdx) {
	return slu::StackItemRef(L, lua_gettop(L), key, parentIdx);
}
slu::StackItemRef Slu_makeStackItemRef(lua_State* L, const std::string& key, const int parentIdx) {
	return slu::StackItemRef(L, lua_gettop(L), key, parentIdx);
}
